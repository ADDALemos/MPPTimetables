...
public:
    ...
    void SetReuseSolver(bool isReuseSolver) { reuseSolver = isReuseSolver; }
    ...
    bool GetReuseSolver() const { return reuseSolver; }
...
private:
    Torc() : polIsConservative(true), conservativeUseAllVars(true), polIsOptimistic(true), varTargetsBumpVal(113),
             bumpRelWeights(false), varTargetsBumpMaxRandVal(0) {};  // Private so that it can  not be called
